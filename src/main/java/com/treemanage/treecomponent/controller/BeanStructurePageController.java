...
public class BeanStructurePageController {
     ...
     * 修改BO属性结构对象信息
     *
     * @param attrStructureInfo
     */
    @RequestMapping("/updateAttrStructureInfo")
    public BaseResponse updateAttrStructureInfo(@RequestBody VOAttrStructureInfo attrStructureInfo) {
        String description = "修改BO结构对象信息";

        VOAttrStructureInfo result = null;
        try {

            AttrStructureInfo bean = parseViewAndBeanModule.parseVO2BO(attrStructureInfo);
            bean = beanStructureBiz.updateAttrStructureInfo(bean);
            result = parseViewAndBeanModule.parseBO2VO(bean);
    ...
    public BaseResponse importBeanStructureInfo(Long parentNodeId, MultipartFile uploadFile) {
        ...
        try {
            ...
            VOBeanStructureInfo voBeanStructureInfo = parseViewAndBeanModule.parseBO2VO((BeanStructureInfo)result.getData());
            result.setData(voBeanStructureInfo);
        } catch (Exception e) {
            description = ReturnUtils.judgeBaseException(e, description);
        }
